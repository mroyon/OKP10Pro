...
{
    public class HrLeaveCancelController : BaseController
    {
        ...
        public async Task<ActionResult> HrLeaveCancel()
        {
            return View("HrLeaveCancelLanding");
        }
        ...
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelTableData([ModelBinder(typeof(DataTablesBinder))] IDataTablesRequest requestModel, hr_leaveinfoEntity input)
        {
            ...
            try
            {
                ...
                List<hr_leaveinfoEntity> data = this.GetAllHrLeaveCancelData(input);
                    ...
                    var tut = (from t in data
                               select new
                               {
                                   ...
                                   ex_nvarchar1 = genButtonPanel(t.leaveinfoid.GetValueOrDefault(-99), "leaveinfoid", t.ex_bigint1.GetValueOrDefault(-99), "leavemodificationid", this.HttpContext.User.Identity as ClaimsIdentity,
                                    t.ex_bigint1.GetValueOrDefault(-99) == -99 && t.iscancel != true && t.isreturn != true && t.leaveinfoid == data.Select(x => x.leaveinfoid).Max() ? "HrLeaveCancel/HrLeaveCancelNew" : "", "HrLeaveCancelNew",
                                    t.ex_bigint1.GetValueOrDefault(-99) != -99 && t.leaveinfoid == data.Select(x => x.leaveinfoid).Max() ? "HrLeaveCancel/HrLeaveCancelEdit" : "", "HrLeaveCancelEdit",
                                   "HrLeaveCancel/HrLeaveCancelDetail", "HrLeaveCancelDetail")
                               }).ToList();
                    ...
                    if (itemList != null && itemList.Count > 0)
                    {
                        ...
                        if (!String.IsNullOrEmpty(actionNameEdit))
                        {
                            if (itemList.Exists(p => p.ActionName.Contains(actionNameEdit)))
                            {
                                ...
                                strValue += "<button  class='btn btn-primary btn-md' onclick ='" + editMethodName + "(&quot;" + objClsPrivate.BuildUrlMVCOnlyParams(menuName, menuId.ToString()) + "&quot;)'> <i class='fa fa-edit'> </i> Cancel </button> ";
        ...
        List<hr_leaveinfoEntity> GetAllHrLeaveCancelData(hr_leaveinfoEntity objhr_leaveinfoEntity)
        {
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelNew(hr_leaveinfoEntity input)
        {
            try
            {
                ...
                ModelState.Clear();
                return PartialView("_HrLeaveCancelNew", model);
            }
            catch (Exception ex)
            {
                return Json(new { status = KAF.MsgContainer._Status._statusFailed, title = KAF.MsgContainer._Status._titleGenericError, redirectUrl = "", responsetext = ex.Message });
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelInsert(hr_leaveinfoEntity input)
        {
            try
            {
                string redirectURL = "";
                ...
                if (input != null && ModelState.IsValid == true)
                {
                    ...
                    {
                        //hr_leavemodificationEntity objLeaveCancell = new hr_leavemodificationEntity();

                        JavaScriptSerializer JsonConvert = new JavaScriptSerializer();
                        string serializeString = JsonConvert.Serialize(objReturnLeave);
                        ...
                        objLeaveCancell.modificationtype = (long?)clsSystemConstantTypes.LeaveModificationType.Cancel;
                        objLeaveCancell.letterdate = input.returnletterdate;
                        objLeaveCancell.letterno = input.returnletterno;
                        objLeaveCancell.remarks = input.remarks;
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelEdit(hr_leavemodificationEntity input)
        {
            SecurityCapsule sec = new SecurityCapsule();
            try
            {
                ...
                ModelState.Clear();
                return PartialView("_HrLeaveCancelEdit", model);
            }
            catch (Exception ex)
            {
                return Json(new { status = KAF.MsgContainer._Status._statusFailed, title = KAF.MsgContainer._Status._titleGenericError, redirectUrl = "", responsetext = ex.Message });
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelUpdate(hr_leavemodificationEntity input)
        {
            try
            {
                string redirectURL = "";
                ...
                if (input != null && ModelState.IsValid == true)
                {
                    ...
                    var objReturnLeaveMod = KAF.FacadeCreatorObjects.hr_leavemodificationFCC.GetFacadeCreate().GetAll(new hr_leavemodificationEntity
                    {
                        leavemodificationid = input.leavemodificationid
                    }).FirstOrDefault();

                    if (objReturnLeaveMod != null)
                    {
                        //hr_leavemodificationEntity objLeaveCancell = new hr_leavemodificationEntity();

                        objReturnLeaveMod.modificationdate = DateTime.Now;
                        //objReturnLeaveMod.modificationtype = (long?)clsSystemConstantTypes.LeaveModificationType.Cancel;
                        objReturnLeaveMod.letterdate = input.letterdate;
                        objReturnLeaveMod.letterno = input.letterno;
                        objReturnLeaveMod.remarks = input.remarks;
                        objReturnLeaveMod.BaseSecurityParam = sec;
                        objReturnLeaveMod.isExt = 1;
                        ret = KAF.FacadeCreatorObjects.hr_leavemodificationFCC.GetFacadeCreate().Update(objReturnLeaveMod);
        ...
        [SecurityFillerAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelDelete(hr_leaveinfoEntity input)
        {
            string str = string.Empty;
            string redirectURL = "";
            Int64 ret = 0;
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> HrLeaveCancelDetail(hr_leaveinfoEntity input)
        {
            SecurityCapsule sec = new SecurityCapsule();
            try
            {
                ...
                ModelState.Clear();
                return PartialView("_HrLeaveCancelDetail", objLeave);
            }
            catch (Exception ex)
            {
                return Json(new { status = KAF.MsgContainer._Status._statusFailed, title = KAF.MsgContainer._Status._titleGenericError, redirectUrl = "", responsetext = ex.Message });
