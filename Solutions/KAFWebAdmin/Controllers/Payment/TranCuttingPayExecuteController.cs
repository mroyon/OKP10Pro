...
{
    public class TranCuttingPayExecuteController : BaseController
    {
        ...
        [ExceptionFilterAttribute]
        public async Task<ActionResult> TranCuttingPayExecuteTableData([ModelBinder(typeof(DataTablesBinder))] IDataTablesRequest requestModel, tran_cuttinginfo_GAPgListView_ExtEntity input)
        {
                ...
                if (data != null && data.Count > 0)
                {
                    ...
                    var tut = (from t in data
                               select new
                               {
                                   ...
                                   t.ItemName,
                                   t.PayAllceID,
                                   strIsRollback = t.isrollback == null ? "" : (t.isrollback == true ? "Yes" : "No"),
                                   ex_nvarchar1 = genButtonPanel(t.CuttingInfoID.GetValueOrDefault(-99), "cuttinginfoid", this.HttpContext.User.Identity as ClaimsIdentity,
                                    t.IsApproved == true ? "" : "TranCuttingPayExecute/TranCuttingPayExecuteEdit", "TranCuttingPayExecuteEdit",
                                   "", "TranCuttingPayExecuteDelete",
                                   "", "TranCuttingPayExecuteDetail", "DownloadReport", t.PayAllceID == 3 ? 3 : 4)
                               }).ToList();
                    ...
                    if (itemList != null && itemList.Count > 0)
                    {
                        ...
                        if (!String.IsNullOrEmpty(actionNameEdit))
                        {
                            if (itemList.Exists(p => p.ActionName.Contains(actionNameEdit)))
                            {
                                ...
                                {
                                    strValue += "<button class='btn btn-primary btn-md' onclick ='" + editMethodName + "(&quot;" + objClsPrivate.BuildUrlMVCOnlyParams(menuName, menuId.ToString()) + "&quot;)'> <i class='fa fa-edit'> </i> Submit</button> ";
                                }
                            }
                        }
        ...
        [ValidateAntiForgeryToken]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> TranCuttingDetailTableData([ModelBinder(typeof(DataTablesBinder))] IDataTablesRequest requestModel, rpt_GetCuttingInfo_ExtEntity input)
        {
            tran_cuttinginfoEntity objowin_tran_cuttinginfo = new tran_cuttinginfoEntity();
            JsonResult result = new JsonResult();
            try
                ...
                if (data != null && data.Count > 0)
                {
                    ...
                    var tut = (from t in data
                               select new
                               {
                                   ...
                                   ex_nvarchar1 = objSecPanel.genButtonPanel(t.CuttingInfoDetlID.GetValueOrDefault(-99), "CuttingInfoDetlID", this.HttpContext.User.Identity as ClaimsIdentity,
                                   "", "UpdatePaymentInfo",
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> TranCuttingPayExecuteEdit(tran_cuttinginfo_GAPgListView_ExtEntity input)
        {
            SecurityCapsule sec = new SecurityCapsule();
            try
            {
                ...
                model.strValue1 = KAF.FacadeCreatorObjects.owin_userFCC.GetFacadeCreate().GetAll(new owin_userEntity { masteruserid=model.ReviewedBy}).FirstOrDefault().username;

                ModelState.Clear();
                return PartialView("_TranCuttingPayExecuteEdit", model);
            }
            catch (Exception ex)
            {
                return Json(new { status = KAF.MsgContainer._Status._statusFailed, title = KAF.MsgContainer._Status._titleGenericError, redirectUrl = "", responsetext = ex.Message });
        ...
        [RequestValidationAttribute]
        [ExceptionFilterAttribute]
        public async Task<ActionResult> TranCuttingPayExecuteUpdate(tran_cuttinginfoEntity input)
        {
            try
            {
                string redirectURL = "";
                ...
                if (model != null)
                {
                    ...
                    if (input.isapproved == true)
                    {
                        ...
                        model.isrollback = null;
                        model.rollbackby = null;
                        model.rollbackdate = null;
                        model.rollbackremarks = "";
                    ...
                    else
                    {
                        ...
                        model.reviewremarks = input.approveremarks;
                        model.isReject = true;
                    }
