...
{
    public class UserPasswordResetController : BaseController
    {
        ...
        public async Task<ActionResult> UserCredentialReset()
        {
            return View();
        }
                    ...
                    if (listobjUser != null && listobjUser.Count > 0)
                    {
                        var dl = (from d in listobjUser
                                  select new
                                  {
                                      ...
                                      passwordkey = d.passwordkey == null ? string.Empty : d.passwordkey,
        ...
        [ExceptionFilterAttribute]
        [RequestValidationAttribute]
        public async Task<ActionResult> ResetUserPassword(owin_userEntity input)
        {
            string str = string.Empty;
            List<owin_userEntity> listobjUser = new List<owin_userEntity>();
            ModelState.Remove("password");
            ...
            try
            {
                if (this.ModelState.IsValid)
                {
                    ...
                    Int64 ret = KAF.FacadeCreatorObjects.FacadeCreatorObjectsPartial.FCCKAFUserSecurity.GetFacadeCreate().UserPasswordResetUpdate(input);
                    if (ret > 0)
                    {
                        GetCountAsync(input);
                        return Json(new { status = KAF.MsgContainer._Status._statusSuccess, title = KAF.MsgContainer._Status._titleInformation, newpass = newpass, redirectUrl = "", responsetext = "User password has been reset successfully." });
                    }
                    else
        ...
        public async Task<string> GetCountAsync(owin_userEntity input)
        {
            ...
            try
            {
                ...
                if (objUser != null)
                {
                    ...
                    obmail.strValue4 = "Your application login credential has been reset from admin. Please collect your credential from admin.";
                    obmail.subject = "Your application login credential has been reset.";
