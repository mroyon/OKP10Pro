...
{
    public class HrAttachmentReturnController : Controller
    {
                ...
                if (data != null && data.Count > 0)
                {
                    ...
                    var tut = (from t in data.OrderByDescending(x => x.attachmentid)
                               select new
                               {
                                   ...
                                   input.CurrentPage == 1 && t.attachmentid == data.Max(x => x.attachmentid) ? "HrAttachmentReturn/HrAttachmentInfoEdit" : "",
                                   input.CurrentPage == 1 && t.attachmentid == data.Max(x => x.attachmentid) ? "HrAttachmentInfoEdit" : "",
                                   "", "",
                                   "HrAttachmentReturn/HrAttachmentInfoDetail", "HrAttachmentInfoDetail")
                               }).ToList();
        ...
        public async Task<ActionResult> HrAttachmentInfoNew(hr_attachmentinfoEntity input)
        {
            try
            {
                ...
                model = KAF.FacadeCreatorObjects.hr_attachmentinfoFCC.GetFacadeCreate().GetAll(new hr_attachmentinfoEntity { hrbasicid = input.hrbasicid }).OrderByDescending(x => x.attachmentid).FirstOrDefault();

                if (model == null || model.returndate != null)
                {
                    throw new Exception("No Attachment Available to Return");
                }

                model.militarynokw = input.militarynokw;
                model.hrbasicid = input.hrbasicid;
