...
$('body').on('click', '#btnSearch', function (event) {
    ...
    if (form.valid()) {
        ...
        try {
            ...
            var divtext = "";
            $.ajax({
                url: baseurl + "UserApproval/GetUserByName",
                data: input,
                type: 'POST',
                success: function (response) {
                    if (response.status === "success") {
                        ...
                        divtext += "<p class='list-group-item-text' style='padding: 2px;'>Is Approved?&nbsp;" + (response.responsetext.approved ? "<input type='checkbox' name='approved' id='approved' checked>" : "<input type='checkbox' name='approved' id='approved' >") + "</p>"
