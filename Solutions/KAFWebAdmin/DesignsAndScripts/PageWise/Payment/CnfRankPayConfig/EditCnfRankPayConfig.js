...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateCnfRankPayConfig', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmRankPayConfigEdit');
            ...
            if (form.valid()) {
                ...
                var input = AddAntiForgeryToken({
                    ...
                    currenturl: window.location.href,
							 ...
							 payconfigid: $('#payconfigid').val(),
							 rankid: $('#rankid').val(),
							 basicsalary: $('#basicsalary').val(),
							 regimentalcutting: $('#regimentalcutting').val(),
							 remarks: $('#remarks').val(),
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "CnfRankPayConfig/CnfRankPayConfig";
                                            $('#mcCnfRankPayConfigEdit').html('');
                                            $('#modal-container-CnfRankPayConfigEdit').modal('hide');
