...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnCreatePageMenuInfo', function (event) {
        try {

            event.preventDefault();

            var form = $('#frmCreatePageMenuInfo');
            ...
            if (form.valid()) {
                ...
                var input = AddAntiForgeryToken({
                    ...
                    , parentmenuid: parseInt($("#ddlpublicmenu").val(), 10)
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: $('#txtBaseUrl').val() + "PageMenuInfo/CreatePageMenuInfo",
                            data: input,
                            type: 'POST',
                            success: function (data) {
                                if (data.status === "success") {
                                    confirmationDialog(data.title, data.responsetext, _getCookieForLanguage("_btnAddMore"), _getCookieForLanguage("_btnGoBackToMain")).then(function (answer) {
                                        if (answer == "true") {
                                            document.getElementById("frmCreatePageMenuInfo").reset();
