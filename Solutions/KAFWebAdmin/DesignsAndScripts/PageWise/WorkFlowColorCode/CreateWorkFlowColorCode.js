...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnCreateWorkFlowColorCode', function (event) {
        try {
            ...
            var form = $('#frmCreateWorkFlowColorCode');
            ...
            if (form.valid()) {
                ...
                var input = AddAntiForgeryToken({
                    ...
                    methodname: "WorkFlowColorCodeCreate",
...
remarks : $('#remarks').val() 
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "WorkFlowColorCode/WorkFlowColorCodeCreate",
                            data: input,
                            type: 'POST',
                            success: function (data) {
                                if (data.status === "success") {
                                    confirmationDialog(data.title, data.responsetext, _getCookieForLanguage("_btnAddMore"), _getCookieForLanguage("_btnGoBackToMain")).then(function (answer) {
                                        if (answer == "true") {
                                            document.getElementById("frmCreateWorkFlowColorCode").reset();
                                            return
                                        }
    ...
    $('body').on('click', '#btnReturn', function (event) {
        try {
            event.preventDefault();
            window.location.href = $('#txtBaseUrl').val() + "WorkFlowColorCode/WorkFlowColorCode";
        } catch (e) {
            $.alert({
                title: _getCookieForLanguage("_informationTitle"),
                content: e.message,
