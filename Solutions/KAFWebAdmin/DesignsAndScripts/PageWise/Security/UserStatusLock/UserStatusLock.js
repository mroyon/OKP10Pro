...
$('body').on('click', '#btnSearch', function (event) {
    ...
    $('#list-panel-lock').empty();
    var form = $('#userStatusLockForm');
    ...
    if (form.valid()) {
        ...
        try {
            ...
            var divtext = "";
            $.ajax({
                url: baseurl + "UserStatusLock/GetUserByName",
                data: input,
                type: 'POST',
                success: function (response) {
                    if (response.status === "success") {
                        ...
                        if (response.responsetext.UserProfilePhoto != '') {
                            ...
                            divtext += "<img id='imgprofileStatusLock' src='" + response.responsetext.UserProfilePhoto + "' style='width: 20%;height:20%;'/>"
                            divtext += "</div>"
                        }
                        ...
                        divtext += "<p class='list-group-item-text' style='padding: 2px;'>Current Status &nbsp;" + (response.responsetext.locked ? "<label>Locked</label>" : "<label>UnLocked</label>") + "</p>"
                       // divtext += "<p class='list-group-item-text' style='padding: 2px;'>Status&nbsp;&nbsp;</p>" + (response.responsetext.locked ? "<input type='button' class='btn btn-primary' value='UnLocked' onclick='UnLock()' >" : "<input type='button' class='btn btn-danger' value='Locked' onclick='Lock()' >") 
                        ...
                        //divtext += "<p class='list-group-item-text' style='padding: 2px;'>Is Locked?&nbsp;" + (response.responsetext.locked ? "<input type='checkbox' name='locked' id='locked' checked>" : "<input type='checkbox' name='locked' id='locked' >") + "</p>"
                        //divtext += "<p class='list-group-item-text'>Comment: </p>"
                        //divtext += "<p class='list-group-item-text'><input type='text' id='txtComment' class='form-control' value='" + response.responsetext.comment + "'></p>"
                        //divtext += "</div>"
                       
                        divtext += "<div class='panel-footer text-center'>"
                        divtext += "<div class='btn-group text-center'>"
                        divtext += "<div class='btn-group'>"
                        divtext += (response.responsetext.locked ? "<input type='button' class='btn btn-primary' value='UnLocked' onclick='UnLock()' >" : "<input type='button' class='btn btn-danger' value='Locked' onclick='Lock()' >") 
                    ...
                    $('#list-panel-lock').append(divtext);
                }
            });
        } catch (e) {
            $.alert({
