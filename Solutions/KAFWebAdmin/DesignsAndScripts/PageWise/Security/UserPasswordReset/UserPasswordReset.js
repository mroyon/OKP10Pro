            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === "success") {
                        ...
                        divtext += "<p class='list-group-item-text'>Type New Password: </p>"
                        divtext += "<p class='list-group-item-text'><input type='text' id='txtPassword' class='form-control txtPassword' ></p>"
                        divtext += "</div>"
                        //divtext += "<div class='list-group-item'>"
                    ...
                    $('#list-panel-password').append(divtext);
                }
            });
        } catch (e) {
            $.alert({
...
$('body').on('click', '#btnProcess', function (event) {
    ...
    try {
        confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_processConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
            if (answer == "true") {
                $.ajax({
                    url: baseurl +"UserPasswordReset/ResetUserPassword",
                    data: input,
                    type: 'POST',
                    success: function (response) {
                        if (response.status === "success") {
                            inforamtionDialog(response.title, response.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                if (answer == "true") {
                                    ...
                                    if (response.newpass == '') {
                                        $("#divnewpasswordtext").append("New Password: " + $('#txtPassword').val());
                                    } else {
