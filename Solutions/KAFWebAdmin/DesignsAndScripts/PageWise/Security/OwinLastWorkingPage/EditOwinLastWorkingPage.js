...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateOwinLastWorkingPage', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmOwin_LastWorkingPageEdit');
            ...
            if (form.valid()) {
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "OwinLastWorkingPage/OwinLastWorkingPageUpdate",
                            data: input,
                            type: 'POST',
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "OwinLastWorkingPage/OwinLastWorkingPage";
                                            ...
                                            $('#modal-container-OwinLastWorkingPageEdit').modal('hide');
