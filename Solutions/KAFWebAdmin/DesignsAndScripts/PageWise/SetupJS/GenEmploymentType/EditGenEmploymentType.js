...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateGenEmploymentType', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmGen_EmploymentTypeEdit');
            ...
            if (form.valid()) {
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "GenEmploymentType/GenEmploymentTypeUpdate",
                            data: input,
                            type: 'POST',
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "GenEmploymentType/GenEmploymentType";
                                            $('#mcGenEmploymentTypeEdit').html('');
                                            $('#modal-container-GenEmploymentTypeEdit').modal('hide');
