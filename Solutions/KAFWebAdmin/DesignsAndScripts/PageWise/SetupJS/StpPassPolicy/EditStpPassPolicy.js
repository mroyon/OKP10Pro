...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateStpPassPolicy', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmStp_PassPolicyEdit');
            ...
            if (form.valid()) {
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "StpPassPolicy/StpPassPolicyUpdate",
                            data: input,
                            type: 'POST',
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "StpPassPolicy/StpPassPolicy";
                                            $('#mcStpPassPolicyEdit').html('');
                                            $('#modal-container-StpPassPolicyEdit').modal('hide');
