...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateGenCurrencyExchageRate', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmCurrencyExchageRateEdit');
            ...
            if (form.valid()) {
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "GenCurrencyExchageRate/GenCurrencyExchageRateUpdate",
                            data: input,
                            type: 'POST',
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "GenCurrencyExchageRate/GenCurrencyExchageRate";
                                            $('#mcGenCurrencyExchageRateEdit').html('');
                                            $('#modal-container-GenCurrencyExchageRateEdit').modal('hide');
