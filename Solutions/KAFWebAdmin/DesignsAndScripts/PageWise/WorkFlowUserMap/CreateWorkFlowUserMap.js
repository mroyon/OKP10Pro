...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnCreateWorkFlowUserMap', function (event) {
        try {
            ...
            var form = $('#frmCreateWorkFlowUserMap');
            ...
            if (form.valid()) {
                ...
                var input = AddAntiForgeryToken({
                    ...
                    methodname: "WorkFlowUserMapCreate",
                    currenturl: window.location.href,
                    entitykey: $("#txtUnit").tokenInput('get')[0].id,
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "WorkFlowUserMap/WorkFlowUserMapCreate",
                            data: input,
                            type: 'POST',
                            success: function (data) {
                                if (data.status === "success") {
                                    confirmationDialog(data.title, data.responsetext, _getCookieForLanguage("_btnAddMore"), _getCookieForLanguage("_btnGoBackToMain")).then(function (answer) {
                                        if (answer == "true") {
                                            document.getElementById("frmCreateWorkFlowUserMap").reset();
                                            return
                                        }
    ...
    $('body').on('click', '#btnReturn', function (event) {
        try {
            event.preventDefault();
            window.location.href = $('#txtBaseUrl').val() + "WorkFlowUserMap/WorkFlowUserMap";
        } catch (e) {
            $.alert({
                title: _getCookieForLanguage("_informationTitle"),
                content: e.message,
