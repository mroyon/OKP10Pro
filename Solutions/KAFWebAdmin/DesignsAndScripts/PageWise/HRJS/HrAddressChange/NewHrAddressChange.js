...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnSaveHrAddressChange', function (event) {
        try {
            event.preventDefault();
            var form = $('#frmHr_AddressChangeNew');
            ...
            if (form.valid()) {
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "HrAddressChange/HrAddressChangeInsert",
                            data: input,
                            type: 'POST',
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "HrAddressChange/HrAddressChange";
                                            $('#mcHrAddressChangeNew').html('');
                                            $('#modal-container-HrAddressChangeNew').modal('hide');
