...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateHrVisaSentInfo', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmHr_VisaSentInfoEdit');
            ...
            if (form.valid()) {
                ...
                //if ($("#GenvisasentDt .chkSelect:checked").length == 0) {
                ...
                //else {
                    confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                        if (answer == "true") {
                            ...
                            $.ajax({
                                url: baseurl + "HrVisaSentInfo/HrVisaSentInfoUpdate",
                                data: input,
                                type: 'POST',
                                    ...
                                    if (data.status === "success") {
                                        inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                            if (answer == "true") {
                                                ...
                                                $('#modal-container-HrVisaSentInfoEdit').modal('hide');
