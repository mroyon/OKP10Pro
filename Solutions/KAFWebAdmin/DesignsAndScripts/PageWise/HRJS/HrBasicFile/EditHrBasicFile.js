...
$(document).ready(function () {
    ...
    $('body').on('click', '#btnUpdateHrBasicFile', function (event) {
        try {
            event.preventDefault();

            var form = $('#frmHr_BasicFileEdit');
            ...
            if (form.valid()) {
                ...
                confirmationDialog(_getCookieForLanguage("_confirmationTitle"), _getCookieForLanguage("_saveConfirmation"), _getCookieForLanguage("_btnYes"), _getCookieForLanguage("_btnNo")).then(function (answer) {
                    if (answer == "true") {
                        ...
                        $.ajax({
                            url: baseurl + "HrBasicFile/HrBasicFileUpdate",
                            data: input,
                            type: 'POST',
                                    ...
                                    inforamtionDialog(data.title, data.responsetext, _getCookieForLanguage("_btnOK")).then(function (answer) {
                                        if (answer == "true") {
                                           //window.location.href =  baseurl + "HrBasicFile/HrBasicFile";
                                            $('#mcHrBasicFileEdit').html('');
                                            $('#modal-container-HrBasicFileEdit').modal('hide');
