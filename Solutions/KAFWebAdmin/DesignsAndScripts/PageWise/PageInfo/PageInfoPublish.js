...
function GetInput() {
    try {
        ...
        var input = AddAntiForgeryToken({
           ...
           // pageinfocategoryid: $('#ddl_pageinfocategory').val() != "" ? parseInt($('#ddl_pageinfocategory').val()) : -99,
            pagetitlear: $('#txtpagetitle').val(),
            ...
            ispublished: $('#ddlIsPublished option:selected').val() == "-99" ? null : $('#ddlIsPublished option:selected').val() == "1" ? true : false
        });
        return input;
...
function GetAllPageInfoData(input) {
    try {
        rev = $('#reviewTable').DataTable(
          {
              ...
              "language":
             {
              ...
              "ajax":
                  {
                      "url": baseurl + "PageInfo/GetPageInfoDataForReviewPublishArchive"
               , "type": "POST"
               , "data": input
              ...
              "columns": [
                        {
                            "data": "pagetitlear", "searchable": true, "sortable": true, "orderable": true,
                            "render": function (data, type, full, meta) {
                                var userid = full.pagetitlear + '<input id="tablepageinfoguid" name="tablepageinfoguid" type="hidden" value="' + full.pageinfoguid + '" />' + '<input id="tablepageinfoid" name="tablepageinfoid" type="hidden" value="' + full.pageinfoid + '" />';
                                return userid;
                            }
                        },
                        { "data": "pagetitle", "searchable": false, "sortable": true, "orderable": true },
                        {
                            "data": "createddate",
                            "render": function (data, type, full, meta) {
                        ...
                        { "data": "isarchived", "searchable": false, "sortable": true, "orderable": true },
                        {
    ...
    try {
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#pageinfoviewdatacontainer').html('');
