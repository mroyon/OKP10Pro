...
{
    public class GenAirlinesController : ApiController
    {
        ...
        [LoggingFilter]
        [ExceptionFilter]
        [Route("api/GenAirlines/getAirlinesData")]
        public async Task<IHttpActionResult> getAirlinesData(string searchTerm, int? pageSize, int? pageNum, string param)
        {
            try
            {
                ...
                gen_airlinesEntity input = new gen_airlinesEntity();
                ...
                var itemList = new List<gen_airlinesEntity>();
                itemList = KAF.FacadeCreatorObjects.gen_airlinesFCC.GetFacadeCreate().GAPgListView(input).ToList();
